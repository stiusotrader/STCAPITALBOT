...
def compute_rs_score(ticker_closes: np.ndarray, spy_closes: np.ndarray) -> float:
    """Relative Strength vs SPY over last 63 trading days (~3 months)."""
...
def analyze_ticker(symbol: str) -> str:
    ...
    try:
        ...
        # EMA 200
        if len(closes) >= 200:
            ema200 = float(np.mean(closes[-200:]))  # simplified EMA as SMA for stability
            # proper EMA
            ema = closes[0]
            k   = 2 / (200 + 1)
            for c in closes:
        ...
        msg += "\n⚠️ _No es asesoramiento financiero._"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Treat message as ticker
    symbol = text.upper().replace(" ", "")
    await update.message.reply_text(f"🔍 Analizando *{symbol}*...", parse_mode="Markdown")
...
async def send_scheduled(bot: Bot, builder_fn):
    try:
        text = builder_fn()
        ...
        logger.info(f"Scheduled message sent: {builder_fn.__name__}")
    except Exception as e:
...
async def main():
    ...
    scheduler.add_job(send_scheduled, "cron", hour=9,  minute=0,  args=[bot, build_opening])
    scheduler.add_job(send_scheduled, "cron", hour=11, minute=0,  args=[bot, build_midmorning])
    scheduler.add_job(send_scheduled, "cron", hour=13, minute=0,  args=[bot, build_midday])
    scheduler.add_job(send_scheduled, "cron", hour=15, minute=0,  args=[bot, build_preclose])
    scheduler.add_job(send_scheduled, "cron", hour=17, minute=0,  args=[bot, build_close])
    scheduler.start()
